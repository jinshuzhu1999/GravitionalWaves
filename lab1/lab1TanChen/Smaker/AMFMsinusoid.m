function sigVec = AMFMsinusoid(dataX,snr,qcCoefs)
%生成信号
% S = AMFMsinusoid(X,SNR,C)Generates a sinusoidal signal S. 
% S =AMFMsinusoid(X,SNR,C)
...
% 2*pi*a1*t;
%相位函数形式1
% 2*pi*a2*t+a3*cos(2*pi*a1*t) ;
%相位函数形式2
%Chen Tan, Feb 2022

phaseVec1 = 2*pi*qcCoefs(1)*dataX;%相位函数1
phaseVec2 =qcCoefs(2)*dataX+qcCoefs(3)*cos(2*pi*qcCoefs(1).*dataX);%相位函数2
