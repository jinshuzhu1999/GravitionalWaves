...
% phaseVec=a1*t+a2*cos(2*pi*a3*t)/(2*pi) ; 相位函数
maxFreq = a1-a2*sin(0)*2*pi*a3./(2*pi);
%相位函数求导后带入t=0
% 绘制信号时间尺度在0-1s内，显然，sin(0)处取得最大频率））
...
sigVec = Frequencymodulated(timeVec,A,[a1,a2,a3]);
sigVec1 = Frequencymodulated(timeVec1,A,[a1,a2,a3]);
sigVec2 = Frequencymodulated(timeVec2,A,[a1,a2,a3]);
