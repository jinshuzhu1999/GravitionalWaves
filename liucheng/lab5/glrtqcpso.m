...
%Determine the number of samples by the number of points in the time series
nSamples = length(inParams.dataX);  

fHandle = @(x) qclrfunc(x,inParams);  %fitness function

nDim = 3;     %dimensional of parameter space
...
for lpruns = 1:nRuns   
    ...
    [templateVec,~] = normsig4psd(estSig,inParams.sampFreq,inParams.psd,1);    
    estAmp = innerprodpsd(inParams.dataY,templateVec,inParams.sampFreq,inParams.psd);  %计算模拟数据与最佳信号的内积，即snr
    [estSig,~] = normsig4psd(estSig,inParams.sampFreq,inParams.psd,estAmp);  
